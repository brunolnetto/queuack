...
def make_job(id):
    return Job(id=id, func=b"", args=b"", kwargs=b"", queue="default", status=JobStatus.PENDING.value)
...
def test_get_and_update_job_status_fields():
    ...
    store.update_job_status("j1", skipped_at=ts, skip_reason="r", skipped_by="u", attempts=2)
